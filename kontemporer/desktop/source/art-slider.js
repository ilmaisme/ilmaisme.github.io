...
function createArrow(node, cur, dir) {
	...
	a.className = 'readSlider-angle -' + dir + ((cur == 0 && dir == 'left') ? ' ' + dis : '');
	a.href = '#';

	let span = document.createElement('span');
	span.className = 'icon-angle icon-angle-' + dir;
	a.appendChild(span);
...
function slideIt(dir) {
	let lbtn = info.querySelector('.readSlider-angle.-left');
	let rbtn = info.querySelector('.readSlider-angle.-right');
	if (!!lbtn && !!rbtn) {
